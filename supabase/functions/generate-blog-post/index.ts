...
// Sanitize HTML generated by AI to fix malformed links and attributes
function sanitizeGeneratedHtml(html: string): string {
  console.log('🧹 Sanitizing generated HTML...');
  
  // Fix 1: Remove double-encoded links like: <a href="url.<a href="...
  html = html.replace(/<a\s+href=["']([^"']*?)<a\s+href=/gi, '<a href="');
  
  // Fix 2: Fix malformed class attributes (class="internal-link">text instead of proper closing)
  html = html.replace(/class=["']([^"']*?)["']>\s*([^<]+)/gi, 'class="$1">$2');
  
  // Fix 3: Remove ALL links from headings (H1-H6) - critical for SEO
  html = html.replace(/<(h[1-6])[^>]*>(.*?)<\/\1>/gi, (match, tag, content) => {
    const cleanContent = content.replace(/<a[^>]*>(.*?)<\/a>/gi, '$1');
    return `<${tag}>${cleanContent}</${tag}>`;
  });
  
  // Fix 4: Remove nested links (<a> inside <a>)
  let hasNestedLinks = true;
  while (hasNestedLinks) {
    const before = html;
    html = html.replace(/<a([^>]*)>(.*?)<a([^>]*)>(.*?)<\/a>(.*?)<\/a>/gi, '<a$1>$2$4$5</a>');
    hasNestedLinks = (before !== html);
  }
  
  // Fix 5: Remove empty or malformed links
...
// Ensure focus keyword appears minimum number of times in content
function ensureKeywordPresence(content: string, keyword: string, targetCount: number = 8): string {
  if (!keyword || keyword.trim() === '') return content;
  
  const keywordRegex = new RegExp(keyword, 'gi');
  const currentMatches = content.match(keywordRegex);
  const currentCount = currentMatches ? currentMatches.length : 0;
  
  console.log(`🔍 Focus keyword "${keyword}": found ${currentCount} times (target: ${targetCount})`);
  
  if (currentCount >= targetCount) {
    console.log('✓ Keyword density is sufficient');
    return content;
  }
  
  const needed = targetCount - currentCount;
  console.log(`⚠️ Need to inject keyword ${needed} more times`);
  
  let updatedContent = content;
  let injected = 0;
  
  // Strategy 1: Add to first paragraph after H1 if not present
  const firstPMatch = updatedContent.match(/(<h1[^>]*>.*?<\/h1>\s*<p>)(.*?)(<\/p>)/i);
  if (firstPMatch && !firstPMatch[2].includes(keyword) && injected < needed) {
    const enhanced = firstPMatch[2].replace(/\.\s/, `. Le ${keyword} est essentiel pour ce sujet. `);
    updatedContent = updatedContent.replace(firstPMatch[0], firstPMatch[1] + enhanced + firstPMatch[3]);
    injected++;
    console.log('✓ Added keyword to first paragraph');
  }
  
  // Strategy 2: Add to 2-3 H2 headings naturally
  const h2Regex = /<h2[^>]*>(.*?)<\/h2>/gi;
  const h2Matches = [...updatedContent.matchAll(h2Regex)];
  
  for (let i = 0; i < Math.min(3, h2Matches.length) && injected < needed; i++) {
    const h2Match = h2Matches[i];
    if (!h2Match[1].includes(keyword)) {
      // Add keyword to H2 naturally
      const keywordPhrase = keyword.split(' ')[0]; // Use first word of keyword
      if (!h2Match[1].toLowerCase().includes(keywordPhrase.toLowerCase())) {
        const enhanced = `${h2Match[1]} avec ${keyword}`;
        updatedContent = updatedContent.replace(h2Match[0], h2Match[0].replace(h2Match[1], enhanced));
        injected++;
        console.log(`✓ Added keyword to H2 #${i + 1}`);
      }
    }
  }
  
  // Strategy 3: Add to last paragraph before conclusion
  const lastPMatch = updatedContent.match(/(<p>)(.*?)(<\/p>\s*<\/[^>]+>\s*$)/i);
  if (lastPMatch && !lastPMatch[2].includes(keyword) && injected < needed) {
    const enhanced = lastPMatch[2].replace(/\.\s*$/, `. En résumé, ${keyword} reste un point crucial. `);
    updatedContent = updatedContent.replace(lastPMatch[0], lastPMatch[1] + enhanced + lastPMatch[3]);
    injected++;
    console.log('✓ Added keyword to last paragraph');
  }
  
  console.log(`✓ Keyword injection complete: added ${injected} occurrences`);
...
✅ RÉÉCRIS-LE complètement avec une approche SEO moderne et des éléments riches (tableaux, FAQ, structure avancée)
    ...
    const systemPrompt = `Tu es un expert en rédaction SEO, HTML propre et application des principes E-E-A-T (Experience, Expertise, Authoritativeness, Trustworthiness) de Google.

RÈGLES HTML CRITIQUES (NON NÉGOCIABLES):
1. ❌ JAMAIS de liens (<a>) dans les titres (H1, H2, H3, H4, H5, H6)
2. ❌ JAMAIS de liens imbriqués (<a> contenant un autre <a>)
3. ❌ JAMAIS d'attributs HTML malformés ou cassés
4. ✅ Chaque lien DOIT avoir un href valide et complet
5. ✅ Chaque lien DOIT avoir un anchor text propre et descriptif
6. ✅ Utilise UNIQUEMENT ces balises HTML: <h1>, <h2>, <h3>, <p>, <ul>, <li>, <strong>, <em>, <figure>, <img>, <figcaption>
7. ✅ Les attributs class doivent être correctement fermés: class="value">

RÈGLES SEO CRITIQUES (NON NÉGOCIABLES):
1. Le mot-clé focus "${keywords[0] || topic}" DOIT apparaître AU MINIMUM 8-12 fois dans l'article
2. Le mot-clé DOIT être présent dans le H1 (titre principal)
3. Le mot-clé DOIT être présent dans au moins 2-3 H2 de manière naturelle
4. Le mot-clé DOIT être présent dans le premier paragraphe (introduction)
5. Le mot-clé DOIT être présent dans le dernier paragraphe (conclusion)
6. Densité du mot-clé cible: 1.5-2% du contenu total
7. Variations du mot-clé encouragées pour éviter la sur-optimisation

${serpAnalysis 
  ? "IMPORTANT: Tu as analysé les concurrents en top 3 de Google - ton objectif est de créer un contenu MEILLEUR qui les surpasse en qualité, profondeur, et utilité pour l'utilisateur."
  : "IMPORTANT: Crée un contenu optimisé 100% pour Google avec une expertise avancée en on-page SEO, sémantique et expérience utilisateur."}`;
...
`;
    ...
    // Call Lovable AI with increased max_tokens for complete content
    console.log(`${logPrefix} 🤖 Calling Lovable AI with google/gemini-2.5-flash (max_tokens: 10000)...`);
    const aiResponse = await fetch('https://ai.gateway.lovable.dev/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        max_tokens: 10000, // Increased to prevent truncated content
        temperature: 0.7,
        messages: [
    ...
    // PHASE 1: Sanitize HTML to fix malformed links and attributes
    console.log(`${logPrefix} 🧹 Sanitizing generated HTML...`);
    finalContent = sanitizeGeneratedHtml(finalContent);
    
    // PHASE 2: Ensure H1 is at the beginning
    if (!finalContent.trim().startsWith('<h1')) {
      console.log(`${logPrefix} ⚠️ H1 not at beginning, fixing...`);
      if (blogPost.title) {
        finalContent = `<h1>${blogPost.title}</h1>\n\n${finalContent}`;
        console.log(`${logPrefix} ✓ H1 inserted at beginning`);
      }
    }
    
    // PHASE 3: Ensure focus keyword presence (minimum 8 times)
    const focusKeyword = keywords[0] || topic;
    console.log(`${logPrefix} 🔍 Validating focus keyword: "${focusKeyword}"`);
    finalContent = ensureKeywordPresence(finalContent, focusKeyword, 8);
    
    // PHASE 4: Final validation
    const finalKeywordCount = (finalContent.match(new RegExp(focusKeyword, 'gi')) || []).length;
    const wordCount = finalContent.split(/\s+/).length;
    console.log(`${logPrefix} ✅ Content ready:`, {
      title: blogPost.title,
      description: blogPost.meta_description?.substring(0, 100),
      contentLength: finalContent.length,
      wordCount: wordCount,
      hasH1: finalContent.includes('<h1>'),
      keywordCount: finalKeywordCount,
      keywordDensity: `${((finalKeywordCount / wordCount) * 100).toFixed(2)}%`
    });
