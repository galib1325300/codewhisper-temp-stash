...
export function BlogSEOScore({ postId, shopId, formData, onOptimizationApplied }: SEOScoreProps) {
  ...
  const handleOptimize = async (category: string, retryCount = 0) => {
    ...
    try {
      ...
      if ((error.message?.includes('Failed to send a request') || error.message?.includes('ERR_CONNECTION_CLOSED')) && retryCount === 0) {
        toast.info('⏳ La requête prend plus de temps que prévu, nouvelle tentative...');
        await new Promise(resolve => setTimeout(resolve, 2000));
        return handleOptimize(category, retryCount + 1);
      }
      ...
      if (error.message?.includes('Failed to send a request') || error.message?.includes('ERR_CONNECTION_CLOSED')) {
        errorMessage = '⚠️ Connexion perdue. La requête a pris trop de temps. Réessayez dans quelques secondes.';
        ...
        errorMessage = '⏱️ Timeout : La requête a pris trop de temps. Essayez une optimisation plus simple.';
